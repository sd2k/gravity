...
use crate::{
    ...
    go::{GoDoc, GoIdentifier, GoImports, GoResult, GoType, comment},
    ...
    pub fn import_chains(&self) -> BTreeMap<String, Tokens<Go>> {
        ...
        for (i, interface) in self.analyzed.interfaces.iter().enumerate() {
            ...
            for resource_name in resource_names {
                ...
                quote_in! { chain =>
                    NewFunctionBuilder().
                    WithFunc(func(ctx $(&self.go_imports.context), mod $(&self.go_imports.wazero_api_module), arg0 uint32) {
                        $(comment(&[
        ...
        for (resource_name, methods) in resource_methods {
            ...
            let method_sigs = methods
                ...
                .map(|method| {
                    ...
                    if params.is_empty() {
                        quote!($method_name(ctx $(&self.go_imports.context)) $return_type)
                    } else {
                        quote!($method_name(ctx $(&self.go_imports.context), $(for p in params join (, ) => $p)) $return_type)
                    }
                })
                .collect::<Vec<_>>();
                    ...
                    if let Some(ret) = &method.return_type {
                        if let GoType::OwnHandle(name) | GoType::BorrowHandle(name) | GoType::Resource(name) = &ret.go_type {
                            ...
                            if method.parameters.is_empty() {
                                sig = quote!($(&method.go_method_name)(ctx $(&self.go_imports.context)) $value_type_param);
                            } else {
                                sig = quote!($(&method.go_method_name)(ctx $(&self.go_imports.context), $(for p in &method.parameters join (, ) => $(&p.name) $(&p.go_type))) $value_type_param);
                            }
                        }
                    }
                } else if is_freestanding {
                    // For freestanding functions, replace resource parameters with type parameters
                    let params_with_ctx = std::iter::once(quote!(ctx $(&self.go_imports.context))).chain(
    ...
    fn generate_host_function_builder(
            ...
            || func_name_str.starts_with("[method]")
        {
            ...
            if let Some(resource_name) = resource_name {
                ...
                Func::import(param_name, result, self.sizes, self.go_imports)
            }
        };
