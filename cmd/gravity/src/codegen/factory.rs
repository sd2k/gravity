...
impl<'a> FactoryGenerator<'a> {
    ...
    fn generate_factory(&self, tokens: &mut Tokens<Go>) {
        ...
        if has_resources {
            ...
            $['\n']
            func (f *$factory_name[$(for (value_param, pointer_param, _) in &type_param_data join (, ) => $value_param, $pointer_param)]) Instantiate(ctx $(&go_imports.context)) (*$instance_name[$(for (value_param, pointer_param, _) in &type_param_data join (, ) => $value_param, $pointer_param)], error) {
            ...
            $['\n']
            func (f *$factory_name[$(for (value_param, pointer_param, _) in &type_param_data join (, ) => $value_param, $pointer_param)]) Close(ctx $(&go_imports.context)) {
                f.runtime.Close(ctx)
            }
            $['\n']
            };
    ...
    fn generate_instance(&self, tokens: &mut Tokens<Go>) {
        ...
        if has_resources {
            ...
            quote_in! { *tokens =>
                ...
                $['\n']
                func (i *$instance_name[$(for (value_param, pointer_param, _) in &type_param_data join (, ) => $value_param, $pointer_param)]) Close(ctx $(&go_imports.context)) error {
        ...
        for (i, (module_name, resources)) in resources_by_module.into_iter().enumerate() {
            ...
            for resource in resources {
                ...
                quote_in! { chain =>
                    ...
                    WithFunc(func(
                        ctx $(&go_imports.context),
                        mod $(&go_imports.wazero_api_module),
                        rep uint32,
                    ) uint32 {
                        $(comment(&[
