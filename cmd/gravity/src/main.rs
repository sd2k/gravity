use std::{collections::BTreeMap, fmt::Write, fs, mem, path::Path, process::ExitCode, str::Chars};
...
#[derive(Debug)]
/// Doc comments.
struct Docs<'a>(&'a wit_bindgen_core::wit_parser::Docs);

impl<'a> FormatInto<Go> for Docs<'a> {
    fn format_into(self, tokens: &mut Tokens<Go>) {
...
impl<'a> std::fmt::Display for Docs<'a> {
...
fn main() -> Result<ExitCode, ()> {
    ...
    let mut content = format!("// Code generated by arcjet-gravity; DO NOT EDIT.\n\n");
    ...
    for (_, world) in &bindgen.resolve.worlds {
        ...
        let docs = Docs(&world.docs);
        write!(content, "{}", docs).unwrap();
        ...
        for (idx, world_item) in world.imports.values().enumerate() {
            match world_item {
                WorldItem::Interface { id, .. } => {
                    ...
                    let iface_docs = Docs(&iface.docs);
    ...
    let mut w = genco::fmt::FmtWriter::new(content);
